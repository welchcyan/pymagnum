...
def read_stock(fileurl):
    ...
    data = {'date': pd.Series(flist[:, 0], index=ind, dtype=pd.Timestamp), 'symbol': pd.Series(flist[:, 1], index=ind),
            'open': pd.Series(flist[:, 2], index=ind, dtype=float), 'high': pd.Series(flist[:, 3], index=ind,dtype=float),
            'low': pd.Series(flist[:, 4], index=ind,dtype=float), 'close': pd.Series(flist[:, 5], index=ind,dtype=float),
